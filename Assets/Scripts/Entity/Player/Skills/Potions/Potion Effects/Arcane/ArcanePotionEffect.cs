...
public class ArcanePotionEffect : BasePotionEffect
{
    ...
    public void CreateCrystal(GameObject obj, Vector3 offset)
    {
        ...
        ArcaneEffectController currentCrystalScript = currentCrystal.GetComponent<ArcaneEffectController>();
    ...
    public Vector3 GetCrystalCoordinate(int index, float heightOffset)
    {
        ...
        {
            throw new System.ArgumentOutOfRangeException(nameof(index), "Index must be between 0 and pointsNumber - 1.");
        }

        // Compute the angle for the vertex in radians
        float angle = 2 * Mathf.PI / amountOfCrystals * index;

        // Adjust the angle to align one base with the x-axis
        float offset = (Mathf.PI / 2) - (Mathf.PI / amountOfCrystals); // Rotate by half the angle between vertices
        angle += offset;
