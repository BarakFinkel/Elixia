...
public class Archer : Enemy
{
    ...
    [SerializeField]
    public float maxDistanceForJump; // The maximal distance from the player to trigger the enemy's jump
    ...
    [SerializeField]
    private Vector2 groundBehinCheckSize;
    ...
    public override void AnimationSpecialAttackTrigger()
    {
        ...
        AudioManager.instance.PlaySFX(38, 0, transform);
    }

    // Helps check if there is ground around the ground check object - will serve checking if the archer can jump back.
    public bool IsGrounBehind()
    {
        return Physics2D.BoxCast(groundBehindCheck.position, groundBehinCheckSize, 0, Vector2.zero, 0, whatIsGround);
    }

    public bool IsWallBehind()
    {
        return Physics2D.Raycast(wallCheck.position, Vector2.right * -facingDir, wallCheckSize.y, whatIsGround);
    }
