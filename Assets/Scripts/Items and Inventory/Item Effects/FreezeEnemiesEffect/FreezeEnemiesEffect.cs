...
{
    [SerializeField]
    private float freezeRadius = 2.0f;

    [SerializeField]
    private float freezeThreshold = 0.2f;
    ...
    public override void ExecuteEffect(Transform _transform)
    {
        ...
        var belowThreshold = stats.currentHealth <= stats.GetMaxHealthValue() * freezeThreshold;
        ...
        if (belowThreshold && Inventory.instance.CanUseArmor())
        {
            ...
            var colliders = Physics2D.OverlapCircleAll(_transform.position, freezeRadius);

            foreach (var hit in colliders) hit.GetComponent<Enemy>()?.FreezeTimeFor(duration);
        }
    }
}